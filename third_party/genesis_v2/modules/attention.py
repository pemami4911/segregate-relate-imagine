...
from third_party.genesis_v2.modules import blocks as B
...
class SimpleSBP(nn.Module):
    ...
    def masks_from_zm_k(self, zm_k, img_size, device):
        # zm_k: K*(batch_size, ldim)
        b_sz = zm_k[0].size(0)
        log_m_k = []
        log_s_k = [torch.zeros(b_sz, 1, img_size, img_size).to(device)]
        other_k = []
...
class LatentSBP(SimpleSBP):
    ...
    def forward(self, x, steps_to_run):
        ...
        stats_k = []
        # Set initial scope to all ones, so log scope is all zeros
        log_s_k = [torch.zeros_like(x, device=x.device)[:, :1, :, :]]
...
class InstanceColouringSBP(nn.Module):
    ...
    def forward(self, features, steps_to_run, debug=False,
        ...
        rand_pixel = torch.empty(batch_size, 1, *colour.shape[2:],device=features.device)
        ...
        for step in range(steps_to_run):
            ...
            seed = torch.empty((batch_size, self.colour_dim),device=features.device)
