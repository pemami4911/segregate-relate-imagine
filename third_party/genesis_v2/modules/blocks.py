...
class PixelCoords(nn.Module):
    ...
    def forward(self, x):
        g_1 = self.g_1.to(x.device)
        g_2 = self.g_2.to(x.device)
        g_1 = g_1.expand(x.size(0), -1, -1, -1).to(x.device)
        g_2 = g_2.expand(x.size(0), -1, -1, -1).to(x.device)
...
class SemiConv(nn.Module):
    ...
    def forward(self, x):
        ...
        delta = out[:, -2:, :, :]
        return out + self.uv.to(x.device), delta
